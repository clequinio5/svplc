...
Namespace SVPLCApplication
	...
	Public Class FormCO0
		...
		Friend Overridable Property BackgroundWorkercomb As BackgroundWorker
			Get
				Return Me._BackgroundWorkercomb
			End Get
			<MethodImpl(MethodImplOptions.Synchronized)>
			Set(ByVal value As BackgroundWorker)
				Dim formCO0 As SVPLCApplication.FormCO0 = Me
				Dim runWorkerCompletedEventHandler As System.ComponentModel.RunWorkerCompletedEventHandler = New System.ComponentModel.RunWorkerCompletedEventHandler(AddressOf formCO0.BackgroundWorker_RunWorkerCompleted)
				Dim formCO01 As SVPLCApplication.FormCO0 = Me
				Dim doWorkEventHandler As System.ComponentModel.DoWorkEventHandler = New System.ComponentModel.DoWorkEventHandler(AddressOf formCO01.BackgroundWorker_DoWork)
		...
		Public Sub New()
			MyBase.New()
			Dim formCO01 As FormCO0 = Me
			AddHandler MyBase.Load,  New EventHandler(AddressOf formCO01.Form6_Load)
			Dim formCO02 As FormCO0 = Me
			AddHandler MyBase.FormClosed,  New FormClosedEventHandler(AddressOf formCO02.FormCO0_FormClosed)
		...
		Private Sub BackgroundWorker_DoWork(ByVal sender As Object, ByVal e As DoWorkEventArgs)
			...
			Do
				...
				Do
					...
					Dim rESULTSASS As Label = Me.tab(1, num3)
					rESULTSASS.TextAlign = ContentAlignment.MiddleCenter
					rESULTSASS.Text = Module1.RESULTSASS(num1 + 1, num3 + 1)
					rESULTSASS.Font = New System.Drawing.Font("Times new roman", CSng(Me.police))
					rESULTSASS.ForeColor = Color.Black
		...
		Private Sub Form6_Load(ByVal sender As Object, ByVal e As EventArgs)
			...
			Dim str() As String = { "Il y a ", Conversions.ToString(Module1.nbrsolutionoptimisees), " solutions optimisées parmis ", Conversions.ToString(Module1.factnumeroparcelle), " possibles et dont la note est de ", Conversions.ToString(Module1.NOTEASSMAX), " sur une échelle allant de -", Conversions.ToString(Module1.nbrfrontierespartagees), " à ", Conversions.ToString(Module1.nbrfrontierespartagees), " ." }
			label1.Text = String.Concat(str)
			If (Module1.nbrsolutionoptimisees <= 120) Then
				Me.nbrsolutionsaffichees = Module1.nbrsolutionoptimisees
			Else
		...
		Private Sub FormCO0_FormClosed(ByVal sender As Object, ByVal e As FormClosedEventArgs)
			Module1.loadcombass = False
		End Sub

		<DebuggerStepThrough>
		Private Sub InitializeComponent()
			Dim componentResourceManager As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(FormCO0))
			...
			Me.Label1.ForeColor = Color.FromArgb(192, 64, 0)
			...
			Me.GroupBoxCombinaisons.ForeColor = Color.FromArgb(255, 128, 0)
			...
			Me.Text = "Combinaisons optimisées année 0"
